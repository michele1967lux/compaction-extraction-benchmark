 ...
 *
 * Ported 1:1 from `dsh-compaction/src/tool-pairing.ts`; the only change is
 * that the session parameter is the injectable {@link ISession} contract
 * instead of the concrete `dsh-session` `Session` (the logic reads exactly
 * `session.surface`, `session.events`, and event `data`).
 *
 * @module ./tool-pairing
 */

import type { ISession, SessionEvent } from './session.ts'
