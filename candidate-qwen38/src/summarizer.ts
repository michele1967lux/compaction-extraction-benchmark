 ...
 *
 * Ported from `dsh-compaction-basic/src/summarizer.ts`.
 *
 * @module ./summarizer
 */
...
import type { CompactionAgentContext } from './session.ts'
 ...
 */
export function frameSummary(summary: readonly ContentBlock[]): readonly ContentBlock[] {
  return [
...
/**
 * Run the default cache-reusing LLM summarization call: replay the
 * conversation prefix, then append the compaction instruction as the final
 * user message so the provider's warm prefix cache is reused.
 * @param llm - the injected LLM client.
...
export async function summarizeWithLlm(
  llm: ILLMClient,
  config: { summarizationProvider: string; summarizationModel: string; maxTokens: number },
  ...
  if (finish === undefined) throw new Error('summarization stream did not finish')
  const error = finishError(finish)
  if (error !== undefined) throw error

  const rawOutput = assembler.getBlocks()
...
/**
 * Minimal block assembler for the summarization stream. Accumulates text
 * deltas, tracks usage, and captures the terminal finish reason.
 */
class BlockAssembler {
  ...
  push(chunk: StreamChunk): void {
    switch (chunk.type) {
      case 'block-start':
        this.currentBlockIndex = chunk.index
        this.currentText = ''
        break
      case 'text-delta':
  ...
  getBlocks(): readonly ContentBlock[] {
    return this.blocks
  }
}
