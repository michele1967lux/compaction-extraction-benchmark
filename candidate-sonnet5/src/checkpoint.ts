/**
 * Compaction checkpoint provenance: the correlated source constructor and
 * type every backend uses for its replacement user message, plus the
 * predicate that recognizes persisted checkpoints.
 *
 * Verbatim port of `packages/compaction/compaction/src/checkpoint.ts`
 * (`@deepseek-ai/dsh-compaction/checkpoint`) — same marker shape, same
 * construction, same predicate.
 *
 * @module checkpoint
 */

import type { CommandId, CompactionId } from './brand.js'
import type { MessageSource } from './session.js'
