 ...
 * positions, so safe cuts are derived from tool-call/result content in
 * current surface order rather than step markers.
 *
 * Verbatim port of `packages/compaction/compaction/src/tool-pairing.ts`
 * (`@deepseek-ai/dsh-compaction/tool-pairing`) — same incremental balance
 * cache, same algorithm — adapted to the injected {@link ISession} instead
 * of the concrete `Session` class.
 *
 * @module tool-pairing
 */

import type { ISession, SessionEvent } from './session.js'
...
const balanceCacheBySession = new WeakMap<ISession, BalanceCache>()
...
function balanceCache(session: ISession): BalanceCache {
 ...
 */
export function toolPairingBalancedBefore(session: ISession, seq: number): boolean {
 ...
 * matching log event, or a tool result has no preceding open call.
 */
export function toolPairingBalancedAfter(session: ISession, seq: number): boolean {
